...
import { getIconMask, uploadMenuItemImage } from '../../../functions';
import Button from '../Base/Button.jsx';
import TextInput from '../Base/TextInput.jsx';
import ToggleButton from '../Base/ToggleButtons.jsx';
import FilePicker from '../FilePicker/FilePicker.jsx';
import MenuCategory from '../Restaurant/MenuCategory.jsx';

const MenuEditorModal = (props) => {
	const { setState: setModalState, editItem, setEditItem } = props,

		[heading, setHeading] = useState(''),
		[stateList, setStateList] = useState([]),
		[menuItemSubmit, setMenuItemSubmit] = useState(true),
		[menuItemSubmitLabel, setMenuItemSubmitLabel] = useState('create'),
		[submitLoading, setSubmitLoading] = useState(false),

		[displayImage, setDisplayImage] = useState(''),
		[image, setImage] = useState(),
		[uploadedImageURL, setUploadedImageURL] = useState(''),
		[uploadPercent, setUploadPercent] = useState(0),
		[categoryName, setCategoryName] = useState(''),
		[selectedCategoryList, setSelectedCategoryList] = useState([]),
		[itemName, setItemName] = useState(''),
		[itemVeg, setItemVeg] = useState('veg'),
		[itemDesc, setItemDesc] = useState(''),
		[itemPrice, setItemPrice] = useState('0'),

		categoryInput = useRef(),
		itemNameInput = useRef(),
		filePicker = useRef(),
		menuEditorModalRef = useRef(),
		menuItemContainer = useRef(),

		maxLengthCategory = 80,
		maxLengthItemName = 55,
		maxLengthItemPrice = 16,
		maxLengthItemDesc = 100,
		...
		titleCase = (value) => (value ? value[0]?.toUpperCase() + value?.slice(1) : ''),
		onClearImage = () => {
			setDisplayImage('');
			setUploadedImageURL();
			setImage();
		...
		onCategorySelect = (categories, change) => {
